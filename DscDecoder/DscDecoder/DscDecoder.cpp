...
{
	unsigned short code;
	unsigned short depth;		/* 该叶节点所在的depth（0开始） */
};
...
int main(int argc, char* argv[])
{
	...
	rewind(file);
	unsigned char* compr = new unsigned char[size];
	fread(compr, 1, size, file);
	dsc_header_t* dsc_header = (dsc_header_t *)compr;
	if (!memcmp(dsc_header->magic, "DSC FORMAT 1.00", 16))
	{
		unsigned char* uncompr = new unsigned char[dsc_header->uncomprlen];
		if (!uncompr)
		{
			delete[] compr;
		...
		unsigned long act_uncomprlen = dsc_decompress(dsc_header, size,
		...
		else
		{
			...
			fwrite(uncompr, 1, dsc_header->uncomprlen, out);
			fclose(out);
		}
		delete[] compr;
	}
